...
awk -v var="$i" 'NR==195 {$0="   log_file_name = \"runs/IB2d.log"'"var"'"\"                //"} 1' input2dw${i}f${i} > input2dw${i}f${i}l${i}
awk -v var="$i" 'NR==201 {$0="   viz_dump_dirname = \"runs/viz_IB2d"'"var"'"\"                //"} 1' input2dw${i}f${i}l${i} > input2dw${i}f${i}l${i}y${i}
awk -v var="$i" 'NR==210 {$0="   data_dump_dirname = \"runs/hier_data_IB2d"'"var"'"\"                //"} 1' input2dw${i}f${i}l${i}y${i} > input2dw${i}f${i}l${i}y${i}j${i}
test=$Freq
# echo $test
if [ $(echo " $test <= 0.70" | bc) -eq 1 ]
then 
    cat template-input2d |  awk 'NR==24 {$0="END_TIME =      3.5                // Final Simulation Time (s)"} 1' input2dw${i}f${i}l${i}y${i}j${i} > input2dw${i}f${i}l${i}y${i}j${i}t${i}
elif [ $(echo " $test < 1.0" | bc) -eq 1 ]
then
    cat template-input2d |  awk 'NR==24 {$0="END_TIME =       3.0               // Final Simluation Time (s)"} 1' input2dw${i}f${i}l${i}y${i}j${i} > input2dw${i}f${i}l${i}y${i}j${i}t${i}
else
    cat template-input2d |  awk 'NR==24 {$0="END_TIME =       2.5                // Final Simulation Time (s)"} 1' input2dw${i}f${i}l${i}y${i}j${i} > input2dw${i}f${i}l${i}y${i}j${i}t${i}
fi
